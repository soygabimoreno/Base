...
class RemoteAudioCourseDataSourceTest {
    private val postService = mockk<PostService>()
    private lateinit var remoteAudioCoursesDataSource: RemoteAudioCourseDataSource

    @Before
    fun setUp() {
        remoteAudioCoursesDataSource = RemoteAudioCourseDataSource(postService)
    }
    ...
    fun `GIVEN audioCourses WHEN getAudioCourses THEN get the expected result`() =
        runTest {
            ...
            val apiModelList: List<AudioCourseApiModel> = relaxedMockk()
