...
class LocalAudioCourseDataSourceTest {
    ...
    private lateinit var dataSource: LocalAudioCourseDataSource
    ...
    @Test
    fun `GIVEN matching audioCourse in DB WHEN getAudioCourseById THEN return mapped audioCourse`() =
        runTest {
            val audioCourseWithItems = buildAudioCourseWithItems()
            coEvery {
                audioCourseTransactionDao.getAudioCourseWithItems(audioCourseWithItems.audioCourseDbModel.id)
            } returns flowOf(audioCourseWithItems)

            val result = dataSource.getAudioCourseById(audioCourseWithItems.audioCourseDbModel.id).first()

            result?.id shouldBeEqualTo audioCourseWithItems.audioCourseDbModel.id
            coVerifyOnce {
                audioCourseTransactionDao.getAudioCourseWithItems(audioCourseWithItems.audioCourseDbModel.id)
            }
        }
    ...
    @Test
    fun `GIVEN no audioCourse in DB WHEN getAudioCourseById THEN return null`() =
        runTest {
            val id = "1-1"
