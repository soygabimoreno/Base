package soy.gabimoreno.presentation.screen.senior
...
fun SeniorScreen(
    ...
    val pullRefreshState =
        rememberPullRefreshState(seniorViewModel.isRefreshing, { seniorViewModel.pullToRefresh() })

    LaunchedEffect(Unit) {
        seniorViewModel.onViewScreen()
    }
    ...
    LaunchedEffect(Unit) {
        seniorViewModel.viewEventFlow.collect { viewEvent ->
            if (viewEvent is SeniorViewModel.ViewEvent.ShowWebView) {
...
private fun EpisodeListContent(
    viewState: SeniorViewModel.ViewState,
    ...
    Box(
        ...
        LazyColumn(state = scrollState) {
            when (viewState) {
                is SeniorViewModel.ViewState.Error -> {
                    item {
                ...
                SeniorViewModel.ViewState.Loading -> {
                    item {
                        LoadingPlaceholder()
                    }
                }

                is SeniorViewModel.ViewState.Success -> {
                    item {
                        EpisodeGrid(
