package soy.gabimoreno.domain.repository.audiocourse
...
import soy.gabimoreno.domain.repository.premiumaudio.TWELVE_HOURS_IN_MILLIS
import soy.gabimoreno.domain.usecase.RefreshPremiumAudiosFromRemoteUseCase
import javax.inject.Inject
import javax.inject.Singleton

@Singleton
class DefaultAudioCourseRepository
    @Inject
    constructor(
        private val cloudDataSource: AudioCourseCloudDataSource,
        private val localAudioCourseDataSource: LocalAudioCourseDataSource,
        private val remoteAudioCoursesDataSource: RemoteAudioCourseDataSource,
        ...
        override suspend fun getAudioCourses(
            ...
            val shouldRefresh =
                forceRefresh ||
                    ...
                    localAudioCourseDataSource.isEmpty()

            if (!shouldRefresh) {
                return localAudioCourseDataSource.getAudioCourses().right()
            }
            return remoteAudioCoursesDataSource
                ...
                .onRight { remoteAudioCourses ->
                    ...
                    localAudioCourseDataSource.saveAudioCourses(mergedCourses)
                }
        }

        override suspend fun getAudioCourseById(
            idCourse: String,
        ): Either<Throwable, Flow<AudioCourse>> {
            val audioCourseFlow = localAudioCourseDataSource.getAudioCourseById(idCourse)
        ...
        override suspend fun getAudioCourseItemById(
            ...
            val audioCourse = localAudioCourseDataSource.getAudioCourseById(audioCourseId).first()
        ...
        override suspend fun markAudioCourseItemAsListened(
            ...
            localAudioCourseDataSource.updateHasBeenListened(audioCourseId, hasBeenListened)
        }
        ...
        override suspend fun reset() {
            localAudioCourseDataSource.reset()
        }
        ...
        override suspend fun updateMarkedAsFavorite(
            ...
            return localAudioCourseDataSource
